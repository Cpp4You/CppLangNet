...
Its highly recommended to view the source code of other existing files before adding a new document.
This allows you to become familiar with the convention and ensure that your new file fits the criteria.
Before doing so, however, we ask that you consult with us first by [filing an issue](https://github.com/Cpp4You/CppLangNet/issues).

:::

## Overview

We're using [Docusaurus](https://docusaurus.io) to write documents. This tool uses an extended markdown format (MDX).
It is a combination of Markdown and JSX, which allows us to write custom components in [React](https://reactjs.org).
We use our [component library](writing-guide/using-components/) to extend the capabilities of what we can put inside a document.
With MDX, we can create documents that are more interactive and engaging for our users than a typical site.
