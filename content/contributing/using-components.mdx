...
```
<SymbolTable noTraits>
	<Symbol name="lineNumbers"	desc="Whether to add line numbers" />
	<Symbol name="maxLines"		desc="Number of lines that when exceeded, causes a vertical scrollbar to appear." />
	<Symbol name="withTitle"	desc="Option that should be added when code block has a title (necessary for proper maxLines calculation" />
