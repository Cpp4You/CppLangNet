...
When it starts the application, it executes the instructions in the `main` code block sequentially. `main` **is always the first code
run by the program.**

The first line here is the **function signature**. It consists of a return type `int`, the name `main`, and its parameters inside the `()` (none).
While most of these terms aren't important now, all that you should pay attention to is the name `main`. The main function will always
look like this, `int main()`. Your program must always contain this function specified in this format.
