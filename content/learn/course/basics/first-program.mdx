...
Strictly speaking, `cout` is a so-called character output stream that writes the content to a *standard output*,
...
After the line that contains `using std::cout;`, the `std::` prefix is no longer required
for the `cout` object. It lasts until the end of the current code block (the `main` function in this case).
