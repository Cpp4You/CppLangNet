---
sidebar_position:	11
sidebar_label:		"11. References (I)"
title:				"References (I)"
description:		"Lesson: references basics in C++ language"
tags:				[reference, pointer, memory]
hide_title:			true
---

1) show that when you assign to a reference it modifies the original value
2) show how to modify vector element in range-based for loop (auto& elem : vec)
3) show that you can add reference in a function parameter and it will modify the original value, not a copy 

# References
References are a way to refer to a variable.
When you create a reference to a variable, it becomes the variable itself.

## Motivations
References are used for *indirection*,
it is so you can access a variable through another variable.

You can create a reference by adding a `&` to the type of the variable you want to refer to.
```cpp
int var = 5;
int& ref = x; // ref is a int reference that refers to var
```
Now, if you modify `ref`, `var` will also be modified because `ref` **is** `var`.
```cpp
ref = 23;
std::cout << var; // Prints out 23
```

Understand but don't see the value?

Some use cases for references are:
- Modifying elements in a range-based for loop
- Passing by reference

## Modifying elements in a range-based for loop
Since references **become** the variable that it is referring to,
we can use it to access the elements in our arrays.

Here is a simple array
```cpp
std::vector<int> arr(3);
arr[0] = 2;
arr[1] = 4;
arr[2] = 6;
```

Let's say we want to add 5 to each element,
It's possible to do that one by one, but it becomes annoying and long if the array is big. <br/>
So loops will come to our rescue!

```cpp
for (int x : arr) {
x = x + 5;
}
// Elements remain unchanged after this loop
```
But the code above will unfortunately not work.
This is because only the value of the element is being written into `x`,
not the element itself.

This is where references come in,
they can make the actual element of the array be written into `x`.
```cpp
for (int& x : arr) {
x = x + 5;
}
```
Now each each element of the array has successfully be increased by five.

## Pass by reference
Function parameters have a similar issue.
They also only copy the value and not the variable itself,
so you can't modify the actual variable.
```cpp
void double(int x) {
x = x * 2; 
}
```

This again, can be solved with references:

```cpp
void double(int& x) {
x = x * 2; 
}
```

:::note Use references accordingly
You should only use references for when you actually need to modify the variable.
Don't use references if you just need the value.
:::
