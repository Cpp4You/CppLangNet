...
The library provides overloads of `std::rint` for all cv-unqualified floating-point types as the type of the parameter `num` <Since v='cpp23'></Since>.
...
The library provides overloads of `std::lrint` and `std::llrint` for all cv-unqualified floating-point types as the type of the parameter `num` <Since v='cpp23'></Since>
