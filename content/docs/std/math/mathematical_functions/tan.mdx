...
Computes the tangent of <code>num</code> (measured in radians). The library provides overloads of `std::tan` for all cv-unqualified floating-point types as the type of the parameter num. <Since v='cpp23'></Since>
<a href='#declarations'>Additional Overloads</a> are provided for all integer types, which are treated as <span class='col-lb'>double</span>. <Since v='cpp11'></Since>
