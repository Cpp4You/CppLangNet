...
If `num1`, `num2` or `num3` has type `long double`, then `std::fma(num1, num2, num3)` has the same effect as 
```cpp
std::fma(static_cast<long double>(num1),
         static_cast<long double>(num2)
         static_cast<long double>(num3)).
```
Otherwise, if `num1`, `num2` and/or `num3` has type `double` or an integer type, `then std::fma(num1, num2, num3)` has the same effect as 
```cpp
std::fma(static_cast<double>(num1),
         static_cast<double>(num2)
         static_cast<double>(num3)).
```
Otherwise, if `num1`, `num2` or `num3` has type `float`, then `std::fma(num1, num2, num3)` has the same effect as
```cpp
...
```cpp
std::fma(static_cast</* common-floating-point-type */>(num1),
