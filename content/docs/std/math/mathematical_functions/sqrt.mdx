...
Computes the square root of `num`. The library provides overloads of `std::sqrt` for all cv-unqualified floating-point types as the type of the parameter `num`. <Since v='cpp23'></Since>
<a href='#declarations'><code><b>Additional Overloads</b></code></a> are provided for all integer types, which are treated as `double`. <Since v='cpp11'></Since>
...
The additional overloads are not required to be provided exactly as <a href='#declarations'><code><b>Additional Overloads</b></code></a>. They only need to be sufficient to ensure that 
