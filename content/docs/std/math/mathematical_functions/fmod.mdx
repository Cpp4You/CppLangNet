...
The expression `x - std::trunc(x / y) * y` may not equal `std::fmod(x, y)`, when the rounding of `x / y` to initialize the argument of <a href='/docs/std/math/mathematical_functions/trunc'><code>std::trunc</code></a> loses too much precision (example: `x = 30.508474576271183309`, `y = 6.1016949152542370172`).
...
`std::fmod(static_cast<long double>(num1), static_cast<long double>(num2))`.
