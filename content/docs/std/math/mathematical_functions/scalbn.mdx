...
Multiplies a floating point value <code>num</code> by `FLT_RADIX` raised to power `exp`. The library provides overloads of `std::scalbn` and `std::scalbln` 
for all cv-unqualified floating-point types as the type of the parameter <code>num</code> <Since v='cpp23'></Since>.
