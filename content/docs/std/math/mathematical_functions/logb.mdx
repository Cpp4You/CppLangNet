...
### Description
Extracts the value of the unbiased radix-independent exponent from the floating-point argument <code>num</code>,  
and returns it as a floating-point value. The library provides overloads of std::logb for all 
cv-unqualified floating-point types as the type of the parameter <code>num</code> <Since v='cpp23'></Since>.
...
is `std::numeric_limits<T>::radix` and `T` is the floating-point type of <code>num</code>. 
