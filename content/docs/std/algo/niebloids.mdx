...
Tche name was suggested by the author himself in a [poll on twitter in 2018](https://twitter.com/tcanens/status/1063723279414890496).
...
`my_ns` does indeed contain `print_me` which accepts `MyEnum::One` as an argument, so it's later found and called.
...
Why am I talking about it? Because it's crucial for understanding Customization Point Objects and then Niebloids.
...
\*\* - One of the characteristics specified is not being able to specify explicit template arguments, it is true with
how niebloids are currently implementated by all the major vendors, but keep in mind that it's not a planned feature of niebloids
...
\*\*\* - People also sometimes mention, that one of the advantages of niebloids over ordinary functions is that you can pass them
...
If you have a need to pass such a function to some HOF, you can use a lambda:
```cpp
#include <iostream>
#include <algorithm>
#include <vector>
...
template <typename Fun>
auto do_thing(std::vector<int>& ints, Fun fun)
{
  fun(ints.begin(), ints.end());
}

auto main() -> int
{
  auto ints = std::vector { 5, 3, 2, 1 };
  do_thing(ints, LIFT(std::sort)); // Works :)
}
```
...
First we define `LIFT_FWD_` which basically does what [`std::forward`](/docs/std/algo/forward) does. If you don't know how
it works, click on the [`std::forward`](/docs/std/algo/forward) link.

We define a macro, because it makes no sense to include the `<utility>` header for our macro.
...
{ return __VA_ARGS__(LIFT_FWD_(args)...); } // the body, actualy function call
```

The only confusing part may be `__VA_ARGS__`, why not just `#define LIFT(X) [...] X(LIFT_FWD_(args)...)`?

The answer is - this would disallow passing in a type with explicit template parameters and a space in it, consider:
```cpp
...
If you are wondering what is the reason behind such a design, I'm here to explain.
