---
title:			any_of algorithm
sidebar_label:	any_of(  )
description:	any_of algorithm C++ documentation
hide_title:		true
tags:			[algorithm, any_of, condition, predicate, any, some]
...
import Signature_SinceCpp11 from './_codes/any_of/since-cpp11.mdx';
import Signature_SinceCpp17 from './_codes/any_of/since-cpp17.mdx';
import Signature_SinceCpp20 from './_codes/any_of/since-cpp20.mdx';
...
- **(1)** Checks if unary predicate `p` returns true for any of the elements in the range [`first`; `last`).
...
`true` if unary predicate returns `true` for any of the elements in the range, `false` otherwise.
