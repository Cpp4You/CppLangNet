---
title:         inclusive_scan algorithm
sidebar_label: inclusive_scan(  )
description:   inclusive_scan algorithm C++ documentation
hide_title:    true
tags:          [algorithm, inclusive_scan, scan, inclusive, fold]

cppreference_origin_rel: w/cpp/algorithm/inclusive_scan
---
...
import Signature_SinceCpp20 from './_codes/inclusive_scan/since-cpp20.mdx';
import Signature_SinceCpp17 from './_codes/inclusive_scan/since-cpp17.mdx';

# std::inclusive_scan() algorithm
...
Computes an inclusive prefix sum operation using `binary_op` (or `std::plus<>()` for overloads **(1, 4)**) for the range [`first`; `last`), using `init` as the initial value (if provided), and writes the results to the range beginning at `d_first`.
...
- If `init` is not provided, `decltype(first)`'s `value_type` must be [`MoveConstructible`](/docs/named_req/MoveConstructible) and `binary_op(*first, *first)` must be convertible to `decltype(first)`'s value type.
