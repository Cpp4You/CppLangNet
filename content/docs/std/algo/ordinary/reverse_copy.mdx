...
If the source and destination ranges (that is, [`first`; `last`) and [`d_first`; `d_first + (last - first`)), respectively) overlap, the <Tooltip term={ <Term_UndefinedBehaviour /> }>behavior is undefined</Tooltip>.
:::
