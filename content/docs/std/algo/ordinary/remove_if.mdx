...
cppreference_origin_rel: w/cpp/algorithm/remove_if
---
...
The standard library also defines an overload of [`std::remove_if`](#) in `<cstdio>`, which takes a `const char*` and is used to delete files.

Because [`std::remove_if`](#) takes `value` by reference, it can have unexpected behavior if it is a reference to an element of the range [`first`; `last`).
...
This is an example of [Erase-remove_if idiom](#).
