...
If [`first`; `last`) is not empty, computes the partial sums of the elements in its subranges and writes the sums to the range beginning at `d_first`,
<Since v='cpp11'>both applying std::move to their operands on the left hand side</Since>.
...
If `op` invalidates any iterators (including the end iterators) or modifies any elements of the range involved, the <Tooltip term={ <Term_UndefinedBehaviour /> }>behavior is undefined</Tooltip>.
:::
