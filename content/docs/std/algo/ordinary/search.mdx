...
- **(5)** Searches the sequence [`first`; `last`) for the pattern specified in the constructor of `searcher`.
  ...
  <SwitchView content={{
    'since-cpp17':
      <SymbolTable>
        <Symbol autoLink linkName="#" name={ <Since v='cpp17'>default_searcher</Since> }>
          Standard C++ library search algorithm implementation.
        </Symbol>
        <Symbol autoLink linkName="#" name={ <Since v='cpp17'>boyer_moore_searcher</Since> }>
          Boyer-Moore search algorithm implementation.
        </Symbol>
        <Symbol autoLink linkName="#" name={ <Since v='cpp17'>boyer_moore_horspool_searcher</Since> }>
