...
A function object type. The program is <Tooltip term={ <Term_IllFormed /> }>ill-formed</Tooltip> if its return type is not one of the three comparison category types ([`std::strong_ordering`](#), [`std::weak_ordering`](#), or [`std::partial_ordering`](#)).
