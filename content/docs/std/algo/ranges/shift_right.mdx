...
cppreference_origin_rel: w/cpp/algorithm/ranges/shift_right
---
...
- **(1)** Shifts the elements towards the end of the range:
  ...
  - If `n < 0`, the <Tooltip term={ <Term_UndefinedBehaviour/> }>behavior is undefined</Tooltip>.
...
The <Tooltip term={ <Term_UndefinedBehaviour/> }>behavior is undefined</Tooltip> if [`first`; `last`) is not a valid range.
:::
