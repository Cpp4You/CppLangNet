...
cppreference_origin_rel: w/cpp/algorithm/ranges/remove_if
---
...
These two invocations together constitute a so-called [Erase–removef idiom](#)<Since v='cpp20'>, which can be achieved by the free function std::erase that has overloads for all standard sequence containers, or std::erase_if that has overloads for all standard containers</Since>
