...
cppreference_origin_rel: w/cpp/algorithm/ranges/replace_if
---
...
Removing is done by shifting (by means of move assignment) the elements in the range in such a way that the elements that are not to be replace_ifd appear in the beginning of the range.
