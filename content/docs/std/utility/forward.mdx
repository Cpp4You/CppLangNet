...
- Writing a **single** constructor which handles **both** lvalue and rvalue arguments and forwards them to its parameters 
  (instead of overloading it on value category of its parameters).

Is is not immediately obvious why such function is necessary. Please refer to [exmaples](#examples) below 
...
// TODO - tutaj przykład z: funkcją robiącą coś z argumentem (dodanie logowania); konstrktorem z wieloma opcjami.
...
To fix this, `person`'s constructor can be overloaded to accept rvalues and move them into fields:
...
While the above example works, it's far from optimal. Consider this case:

```cpp
auto p3 = person(name, "Bar");
```

The creation of `p3` cannot invoke the constructor that takes two rvalues (and moves from them), because `name` is an lvalue.
Thus the only candidate is the constructor taking two strings by `const&`. Resources are wasted to create a temporary for `"Bar"` and copy it.
...
Instead of doing so, we can turn those constructors into a single `tempalate`:
...
The tricky part is what to put instead of `/* ??? */`. We could put *nothing* there, but we would never use move constructors (which we agreed is not optimal).
We can't put `std::move()` there, because in the case of receiving an lvalue, we shouldn't move from it.
...
Despite that, `name` and `surname` are parameters that have names and thus, if we can name them, they are treated as lvalues.
