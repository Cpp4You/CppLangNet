...
The new string can be one of:
 ...
 - **(3)** Characters in the range **[ first2, last2 )**.
	<SwitchView content={{
		'since-cpp11': <>
			This overload only participates in overload resolution if <code>InputIt</code> qualifies as an <a href="/docs/named_req/LegacyInputIterator"><code>LegacyInputIterator</code></a>.
		</>,
		'until-cpp11': <>
			This overload has the same effect as overload <b>(6)</b> if <code>InputIt</code> is an integral type.
		</>
	}} />
