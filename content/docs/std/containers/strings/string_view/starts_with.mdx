...
```cpp
std::basic_string_view_view<CharT, Traits>(data(), size()).starts_with(x)
```
...
int main()
{
    ...
    test_prefix_print(helloWorld, std::string_view_view("hello"));
 
    test_prefix_print(helloWorld, std::string_view_view("goodbye"));
