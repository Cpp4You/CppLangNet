...
The only usage cases are in functions [`insert_after()`](insert-after), [`emplace_after()`](emplace_after), [`erase_after()`](erase_after), [`splice_after()`](splice_after) and the increment operator: incrementing the before-begin iterator gives exactly the same iterator as obtained from [`begin/cbegin()`](begin).
:::
