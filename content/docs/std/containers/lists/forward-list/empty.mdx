...
it's likely that the programmer might confuse the adjective `empty` (which would mean - **is this container empty?**) for the verb `empty` (which would mean - **please empty this container for me.**).
