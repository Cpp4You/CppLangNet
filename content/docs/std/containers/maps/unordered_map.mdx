...
tags:				[unordered_map, associative, container, key]
hide_title:			true
---
...
The `std::unordered_map` is a container that stores key-value pairs, and unlike `std::map` is not ordered by key/value.
### Example usage
...
<SymbolTable>
	...
	<Symbol pub autoLink linkName="operator[]" name="operator[]">
		Access oer insert specified element
	</Symbol>
...
int main() {
  ...
  for (auto& [key, value] : player_locations)
    std::cout << key << " is in " << value << std::endl;
