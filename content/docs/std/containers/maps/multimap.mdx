...
<SymbolTable>
	<Symbol pub autoLink linkName="value_comp" name="value_compare">
...
<SymbolTable>
	<Symbol pub autoLink linkName="empty" name="empty">
		Returns <code>true</code> if a multimap is empty, otherwise <code>false</code>.
	</Symbol>
...
<SymbolTable>
	...
	<Symbol pub autoLink linkName="contains" name={<>contains <small>(since C++20)</small></>}>
		Returns <code>true</code> if an element is inside a multimap, otherwise <code>false</code>.
	</Symbol>
...
<SymbolTable noTraits>
	...
	<Symbol name={<>std::swap <small>(std::multimap)</small></>}
		...
		/>
	<Symbol name={<>
				std::erase_if <small>(std::multimap) (since C++20)</small>
			</>}
