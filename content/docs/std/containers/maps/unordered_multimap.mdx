...
tags:				[unordered_multimap, associative, container, key, multi, unordered]
---
...
<!-- Further Examples -->
import FurtherExampleCreateInsertPrint from "./_codes/unordered-multimap/main-page/examples/further/create-insert-print.mdx"

<!-- Overview -->
import OverviewSimplifiedRegular from "./_codes/unordered-multimap/main-page/overview/overview-simplified-regular.mdx";
import OverviewDetailedRegular from "./_codes/unordered-multimap/main-page/overview/overview-detailed-regular.mdx";
import OverviewDetailedPmr from "./_codes/unordered-multimap/main-page/overview/overview-detailed-pmr.mdx";

<!-- Deduction Guides -->
import DeductionGuides from "./_codes/unordered-multimap/main-page/deduction-guides.mdx"
...
`std::unordered_multimap`, the only difference is that the keys can repeat themselves.
...
<SymbolTable>
	...
	<Symbol pub autoLink linkName="destructor" name="(destructor)">
		Destructs a unordered_multimap.
	</Symbol>
...
<SymbolTable>
	<Symbol pub autoLink linkName="empty" name="empty">
		Returns <code>true</code> if the unordered_multimap is empty, otherwise <code>false</code>.
	</Symbol>
...
<SymbolTable>
	<Symbol pub autoLink linkName="clear" name="clear">
		Clears the contents of a unordered_multimap.
	</Symbol>
	...
	<Symbol pub autoLink linkName="extract" name={<>extract <small>(since C++17)</small></>}>
		Extracts nodes from the unordered_multimap (can be later inserted somewhere else).
	</Symbol>
...
<SymbolTable>
	<Symbol pub autoLink linkName="count" name="count">
		Returns the number of elements matching a specific key (for unordered_multimap it's always going to be 1 or 0).
	</Symbol>
	...
	<Symbol pub autoLink linkName="contains" name={<>contains <small>(since C++20)</small></>}>
		Returns <code>true</code> if an element is inside the unordered_multimap, otherwise <code>false</code>.
	</Symbol>
	<Symbol pub autoLink linkName="equal_range" name="equal_range">
		Returns a range of elements matching a specific key (for unordered_multimap it's always going to be one or zero elements).
	</Symbol>
</SymbolTable>
...
<SymbolTable noTraits>
	...
	<Symbol name={<>std::swap <small>(std::unordered_multimap)</small></>}
		...
		/>
	<Symbol name={<>
				std::erase_if <small>(std::unordered_multimap)</small>
			</>}
