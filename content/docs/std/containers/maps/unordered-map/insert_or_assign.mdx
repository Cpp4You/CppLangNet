...
<SwitchView content={{
 ...
 - **(1-3)** 
    ...
    If the key does not exist, inserts the new value as if by [`insert()`](/docs/maps/unordered-map/insert), constructing it from `value_type(k, std::forward<M>(obj))`.
...
[`insert_or_assign`](#) returns more information than [`operator[]`](/docs/std/containers/maps/unordered-map/operator_subscript) and does not require default-constructibility of the mapped type.
