...
Inserts a new element into the container with key `k` and value constructed with `args`, if there is no element with the key in the container.
 ...
 - **(1)** 
    If a key equivalent to `k` already exists in the container, does nothing. Otherwise, behaves like [`emplace()`](/docs/std/containers/maps/unordered-map/emplace) except that the element is constructed as
    ```cpp
 ...
 - **(3)** 
    If a key equivalent to `k` already exists in the container, does nothing. Otherwise, behaves like [`emplace_hint()`](/docs/std/containers/maps/unordered-map/emplace_hint) except that the element is constructed as
    ```cpp
    value_type(std::piecewise_construct,
               std::forward_as_tuple(k),
               std::forward_as_tuple(std::forward<Args>(args)...))
...
### Return value
 ...
 - **(1-2)** Same as for [`emplace()`](/docs/std/containers/maps/unordered-map/emplace).
 - **(3-4)** Same as for [`emplace_hint()`](/docs/std/containers/maps/unordered-map/emplace_hint).
...
Unlike [`insert()`](/docs/std/containers/maps/unordered-map/insert) or [`emplace()`](/docs/std/containers/maps/unordered-map/emplace),
