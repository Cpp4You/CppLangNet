...
Iterator to an element with key equivalent to `key`. If no such element is found, past-the-end (see [`end()`](/docs/std/containers/maps/unordered-map/end)) iterator is returned.
