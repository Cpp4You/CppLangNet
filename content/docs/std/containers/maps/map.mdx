...
The `std::map` is a container that stores key-value pairs with unique keys in some order. The order can be specified by the programmer. 
...
int main(){
  std::map<std::string, signed int> player_stats{ {"C1", -10}, {"B2", 25}};
