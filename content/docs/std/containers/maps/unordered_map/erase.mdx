...
The iterator pos must be valid and dereferenceable. Thus the [`end()`](/docs/std/containers/maps/unordered_map/erase) iterator (which is valid, but is not dereferenceable) cannot be used as a value for `pos`.
:::
