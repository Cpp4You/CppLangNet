...
### Type requirements
 ...
 - `T` *(the container's element type)* must meet the requirements of [`MoveInsertable`](/docs/named_req/MoveInsertable) and [`EmplaceConstructible`](/docs/named_req/DefaultInsertable).
...
If `T`'s move constructor is **not noexcept** and `T` is not [`CopyInsertable`](/docs/named_req/CopyInsertable) into `*this`,
vector will use the throwing move constructor. If it throws, the guarantee is waived and the effects are unspecified.

### Notes

Since reallocation may take place, [`emplace_back()`](emplace_back) requires the element type to be [`MoveInsertable`](/docs/named_req/MoveInsertable) for vectors.
