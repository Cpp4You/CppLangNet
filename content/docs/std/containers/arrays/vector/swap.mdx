...
<SwitchView content={{
	'since-cpp11': <>
		...
		Otherwise, they are not swapped (and if <code>get_allocator() != other.get_allocator()</code>, the <Tooltip title={Term_UndefinedBehaviour}>behavior is undefined</Tooltip>).
	</>
}} />
