...
int main()
{
    ...
    auto it = arr.rend(); // Type: std::array<int>::reverse_iterator
    // highlight-next-line
    *std::prev(it) = 5; // ✔ Ok
}
```
...
{
    std::array<int> arr = {1, 2, 3, 4, 5};
    auto it = arr.crend(); // Type: std::array<int>::reverse_const_iterator
    // highlight-next-line
    *std::prev(it) = 5; // ❌ Error! 
}
```
...
int main()
{
    ...
    auto it = arr.rend(); // Type: std::array<int>::reverse_const_iterator
    // highlight-next-line
    *std::prev(it) = 5; // ❌ Error! 
}
```
