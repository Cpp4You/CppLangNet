...
The returned value is valid only for instances of the container for which [`bucket_count()`](/docs/std/containers/sets/unordered_multiset/bucket_count) returns the same value.

:::danger 
The behavior is **undefined** if [`bucket_count()`](/docs/std/containers/sets/unordered_multiset/bucket_count) is zero.
:::
