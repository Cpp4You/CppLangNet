---
title:					multiset<...>::lower_bound() method
...
Iterator pointing to the first element that is **not less** than `key`. If no such element is found, a past-the-end iterator (see [`end()`](/docs/std/containers/sets/multiset/end)) is returned.
