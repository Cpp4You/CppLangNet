...
This makes it possible to create generic inserters such as [`std::inserter()`](#). One way to check success of a hinted insert is to compare [`size()`](/docs/std/containers/sets/unordered-set/size) before and after. 
