...
The iterator pos must be valid and dereferenceable. Thus the [`end()`](/docs/std/containers/unordered-multiset/erase) iterator (which is valid, but is not dereferenceable) cannot be used as a value for `pos`.
:::
...
### Complexity
 ...
 - **(3)** 
  Average case: **`count(key)`**

  Worst case: **Linear in the size of the container**

 - **(4)** 
  Average case: **`count(x)`**
