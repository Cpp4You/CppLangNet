...
Iterator to an element with key equivalent to `key`. If no such element is found, past-the-end (see [`end()`](/docs/std/containers/sets/unordered_set/end)) iterator is returned.
