...
|:----------:|:------------------------------:|:--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------:|
| `a == b`     | [implicitly convertible](language/implicit_conversion) to bool | Establishes an [equivalence relation](https://en.wikipedia.org/wiki/Equivalence_relation), that is, it satisfies the following properties:<br/> \* For all values of `a`, `a == a `yields `true`.<br/> \* If `a == b`, then `b == a` <br/> \* If `a == b` and `b == c`, then `a == c` |
...
For the types that are both *EqualityComparable* and [*LessThanComparable*](/docs/std/named_req/LessThanComparable), 
