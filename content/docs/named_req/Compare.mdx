...
to `bool`, yields `true` if the first argument of the call appears before the second in the 
[strict weak ordering relation](https://en.wikipedia.org/wiki/Weak_ordering#Strict_weak_orderings) induced by this type, and `false` otherwise.
