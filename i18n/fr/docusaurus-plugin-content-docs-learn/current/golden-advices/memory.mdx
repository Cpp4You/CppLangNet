...
Dans l'aménagement de la mémoire nous pouvons effectuer beaucoup d'erreurs. Les conseils ci-dessous vous aideront à en faire le moins possible 😄

## Exactitude et sécurité

### Utilisez des références

La règle est simple: si un fragment de code **demande**, qu'un objet existe, utilise une référence et pas un pointer.

#### Exemple

Nous avons une class d'un joueur 👨‍💼:
...
Nous voulons créer une fonction, qui nous permet d'attaquer un autre joueur:
...
Les pointers brutes (par ex. `int* ptr`) ont beaucoup de controverse. Ça vaut la peine de savoir quand on peut l'utiliser.

Pour commencer:

:::tip
Il n'y a aucun problème dans l'utilisation des pointers brutes... de la manière vue précédemment.
:::

Le rôle d'un pointer brute est **seulement** l'accès à un endroit précis dans la mémoire.

:::danger
N'utilisez pas de pointer brute pour **aménager la mémoire** (tant que vous n'avez pas de certitude de ce que vos faites).
