...
Wiążę się to z dwoma rzeczami: kolejność wykonywania elementów pętli `for` i działania typów bez znaku (`unsigned`).

#### Kolejność wykonywania

Tak jak poznaliśmy wcześniej, kolejność wykonywania elementów w pętli `for` jest następująca:
```cpp title="🔹 Kolejność wykonywania elementów for"
for( /* 0 */; /* 1 */; /* 3 */ ) 
{
	/* 2 */
}
...
Instrukcja początkowa jest specjalnie zaznaczona numerem **0**. Wykonuje się ona tylko raz na początku pętli.
Następnie powtarzane są następujące akcje:
 - Jeśli warunek (**1**) jest spełniony, to wykonaj ciało (**2**) i wyrażenie iteracji (**3**)
 - Jeśli warunek (**1**) nie jest spełniony, to pomiń pętlę

Akcje te wykonują się do momentu, do którego warunek nie jest spełniony.

Także proces wykonywania się pętli może przybrać dwie formy:
 - **0** 123... **4** - kiedy warunek pętli jest spełniony przynajmniej raz
 - **0** 1 **4** - kiedy warunek pętli od razu będzie fałszywy

#### Zawijanie liczb bez znaku

Liczby bez znaku cechują się zachowaniem, które można opisać jako zawijanie (*ang. wrapping*).
`std::size_t` to alias na liczbę bez znaku,
która w teorii ma być zdolna do przechowania wielkości obiektu największego możliwego obiektu na danej architekturze,
lub indeksacji największej możliwej tablicy - dlatego metoda `.size()` tablicy czy napisu zwraca właśnie wartość tego typu.
