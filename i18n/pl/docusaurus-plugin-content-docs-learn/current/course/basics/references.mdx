...
Teraz kiedy funkcja `doubleNumber` przyjmuje referencję, `num` stanie się zmienną przekazaną do funkcji. Oznacza to, że jeśli zmodyfikujemy `num` (który stał się `x`), `x` także ulegnie zmianie.
