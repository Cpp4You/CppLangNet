...
int main() {
	...
	// Możemy dodawać napisy do siebie:
...
```plaintext title="Wynik (konsola)"
Mamy dzisiaj 3. dzień tygodnia, dzisiejsza temperatura to 17.000000 stopni Celsjusza!
```
...
```plaintext title="Wynik (konsola)"
Standard c++20 wyszedł w roku 2022, 3 lata po standardzie C++17.
```
...
N a z y w a m   s i  e     S  u  p  e  r     C  o  d  e  r
                           ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
```
`introduction.find("Super Coder")` znajdzie nam pozycję `"Super Coder"`, zwróci indeks **12**.
...
int main() {
	std::string s = 'Hej!';
	// Bardzo mozliwe, ze osobno dostaniemy komunikat o 'Hej!'
	// i osobno o samej probie przypisania (patrz Tresc bledu ponizej):
}
```
...
W C++ie napisy zapisujemy pomiędzy znakami `""`. Jeśli spróbujemy wypisać lub zapisać w `std::string` w normalny sposób `"`:

```cpp title='Niepoprawny kod'
std::string sentence = "Waran z Komodo inaczej nazywany "smokiem z Komodo" jest najwieksza zyjaca jaszczurka.";
```

To program analizujący składnię C++ zintepretuje to jako:
```
"Waran z Komodo inaczej nazywany "smokiem z Komodo" jest najwieksza zyjaca jaszczurka."
               NAPIS                KOD C++                NAPIS
```
...
Jeden *escape sequence* już poznaliśmy - to `\n`, które powoduje zrobienie nowej linii.
...
```plaintext
Wprowadz swoj wiek: 15
Wprowadz swoje imie:  masz 15lat.
```

Wyżej wywołany został program, użytkownik został poproszony o wiek, więc go wprowadził.
Po wprowadzeniu wieku, zostało wyświetlone `Wprowadz swoje imie: masz 15 lat.`
...
```plaintext
Wprowadz swoj wiek: 15
Wprowadz swoje imie: Super Coder
