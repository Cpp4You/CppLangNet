...
a wewnątrz nawiasów argumenty, które chcemy podać, oddzielone przecinkiem: `std::cin, fullName`.
...
Zauważ, że `coolName` jest wynikiem *konkatenacji* trzech łańcuchów - `firstName`, `"The Rock"` i `lastName`.
Dalej w kodzie można zobaczyć, jak modyfikujemy `coolName` poprzez *dołącznie* do niego `" Mk II"`.
