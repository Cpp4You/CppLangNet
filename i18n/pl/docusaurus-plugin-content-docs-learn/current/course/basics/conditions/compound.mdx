...
{
	// Kod w środku wykonuje się tylko wtedy, kiedy poprzedni warunek był fałszywy
	// i obecny warunek jest prawdziwy
}
else
{
...
Możemy to łatwo zrobić, dzięki temu, że ustawiliśmy `'X'` jako domyślną wartość `licenseClass`. Oznacza to, że każdy kto
nie otrzymał prawa jazdy, nadal będzie miał `'X'` jako wartość `licenseClass`, a my możemy to sprawdzić za pomocą warunku.
...
Stworzyliśmy tutaj nową instrukcję if, która sprawdza czy `licenseClass != 'X'`.
...
W naszym programie cała logika do określania `licenseClass` oraz `illegalReason` znajduje się w jednym ifie,
