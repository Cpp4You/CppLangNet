---
sidebar_position:	3
sidebar_label:		"3. Zmienne"
description:		"Lekcja: zmienne w języku C++"
---
...
- `int` - typu zmiennej
- `x` - nazwy zmiennej
- `= 212` - **inicjalizacji** (przypisania początkowej wartości)
- `;` - średnika, który kończy instrukcję
...
```cpp
// Bez przypisania początkowej wartości
...
musimy zakończyć tą wartość znakiem `f`:
- ✔ `12.45f`
- ❌ `12.45`
...
Nazwa zmiennej jest jej unikalnym identyfikatorem (z pewnymi wyjątkami, ale o nich kiedy indziej).
...
`cin` pozwala użytkownikowi programu wpisać coś od konsoli,
...
użycia operatora `<<`. Polecamy tą technikę 😀
:::
...
<CustomCodeBlock lineNumbers withTitle maxLines={15}>
...
int main()
{
	...
	// Zmniejsz "value" o 10:
	number -= 10;
	std::cout << "Teraz, po zmniejszeniu o 10: " << number << '\n';

	// Pomnóz trzykrotnie:
	number *= 3;
	std::cout << "Teraz, po pomnożeniu razy 3: " << number << '\n';

	// Podziel na 2:
	number /= 2;
	std::cout << "Teraz, po podzieleniu przez 2: " << number << '\n';

	// Zwiększ wartość o 1 (inkrementacja):
	number++;
	std::cout << "Teraz, po zinkrementowaniu: " << number << '\n';

	// Zmniejsz wartość o 1 (dekrementacja):
	number--;
	std::cout << "Teraz, po zdekrementowaniu: " << number << '\n';
}
```
