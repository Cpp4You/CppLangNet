...
Lambdy służa do tworzenia obiektów funkcyjnych, które można wywołać jak funkcje. 
...
```
Podstawowa lambda musi posiadać conajmniej ciało i listę wychwytywania. Często spotyka się też listę argumentów.
...
## Czym są te całe lambdy?
Lambdy służa do tworzenia obiektów funkcyjnych, które można wywałoać. 

:::important *"Nienazwane funkcje"*
Lambdy często nazywane w ten sposób, lecz nie jest to poprawna nazwa. Lambdy mogą być obiektem z nazwą zdefiniowanym za pomocą słowa kluczowego `auto` lub typu `std::function` (o którym dowiemy się w lekcji drugiej o lambdach).
:::

## Do czego mogą nam się przydać?
Lambdy przydają się często do algorytmów z biblioteki standardowej. W drugiej części lekcji o lambdach dowiemy się jak stworzyć taki własny algorytm.
...
#### 1. Lista wychwytywania (ang. capture list)
Jak wiemy z lekcji o funkcjach, zmienne lokalne (np. z funkcji main) nie są znane w ciele innej funkcji. To samo tyczy się wyrażeń lambda. Zmienne lokalne dodane na listę wychwytywania są znane w wyrażeniu lambda.
...
std::cout << five();
```
Powyższy kod wypisze na ekranie: ```5```

Następujący kod zawiera wyrażenie lambda z argumentem:
```cpp
auto square = [](int x) { return x*x; };
std::cout << square(5);
```
Nasz obiekt funkcyjny `square` zwraca sześcian argumentu `x`.
Rezultat programu: `25`
