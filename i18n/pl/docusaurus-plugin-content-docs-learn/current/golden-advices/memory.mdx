...
Do kontroli czasu życia obiektu użyj **inteligentnego wskaźnika** (*ang.: smart pointer*):
...
Jeśli chcesz przekazać cały obiekt do jakiegoś rejestru/magazynu/*managera* (NIE NADUŻYWAJMY `XYZManager`),
...
Jeśli nie potrzebujesz kopiować obiektu, przekaż go przez (stałą) referencje.
...
```cpp title="❌ ŹLE"
void print(Vector3f vec) // vec zostaje skopiowany do parametru funkcji
{
...
```cpp title="✔ DOBRZE"
void print(Vector3f const& vec) // stała referencja, nie potrzebujemy tutaj kopiować
{
	std::cout << vec.x << ", " << vec.y << ", " << vec.z;
}
```
