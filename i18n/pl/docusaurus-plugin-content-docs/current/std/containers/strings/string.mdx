---
sidebar_position: 2
title: string
---

# Klasa string

import Class from "@site/src/components/mdx/ClassSummary";
import Columns from "@site/src/components/mdx/Columns";
import NotFinished from "@site/i18n/en/presets/NotFinished.mdx";
import ImproveSection from "@site/i18n/en/presets/ImproveSection.mdx";

<NotFinished />

A class that is used to store and manipulate sequence of characters.

The elements of a string are stored contiguously in memory.


<details>
<summary>Technical definition of a string</summary>
<div>

The class template `basic_string` stores and manipulates sequences of `char`-like objects,
which are non-array objects of [trivial](/docs/named_req/triavial_type)
[standard-layout](/docs/named_req/standard_layout_type) type. The class is dependent neither
on the character type nor on the nature of operations on that type.
The definitions of the operations are supplied via the Traits template parameter - a specialization
of [`std::char_traits`](char_traits) or a compatible traits class. `Traits::char_type` and `CharT`
must name the same type; otherwise the program is ill-formed.

The elements of a basic_string are stored contiguously, that is, for a basic_string s,
`&*(s.begin() + n) == &*s.begin() + n` for any n in `[0, s.size())`, or, equivalently,
a pointer to `s[0]` can be passed to functions that expect a pointer to the first element
of a null-terminated <small>(since C++11)</small> `CharT[]` array.

</div>
</details>

:::note
Type `std::string` is in fact an alias to: `std::basic_string<char>`.

`std::basic_string` is a class template that allows you to use it with
many different character types.

<details>
	<summary>Available <code>std::basic_string</code> aliases</summary>
	<div>
		<Class.SymbolTable noTraits>
			<Class.TypeName name="std::string"			desc={<code>std::basic_string&lt;char&gt;</code>}/>
			<Class.TypeName name="std::wstring" 		desc={<code>std::basic_string&lt;wchar_t&gt;</code>}/>
			<Class.TypeName name="std::u8string"		desc={<code>std::basic_string&lt;char8_t&gt;</code>}/>
			<Class.TypeName name="std::u16string"		desc={<code>std::basic_string&lt;char16_t&gt;</code>}/>
			<Class.TypeName name="std::u32string"		desc={<code>std::basic_string&lt;char32_t&gt;</code>}/>
			<Class.TypeName name="std::pmr::string"		desc={<code>std::pmr::basic_string&lt;char&gt;</code>}/>
			<Class.TypeName name="std::pmr::wstring" 	desc={<code>std::pmr::basic_string&lt;wchar_t&gt;</code>}/>
			<Class.TypeName name="std::pmr::u8string"	desc={<code>std::pmr::basic_string&lt;char8_t&gt;</code>}/>
			<Class.TypeName name="std::pmr::u16string"	desc={<code>std::pmr::basic_string&lt;char16_t&gt;</code>}/>
			<Class.TypeName name="std::pmr::u32string"	desc={<code>std::pmr::basic_string&lt;char32_t&gt;</code>}/>
		</Class.SymbolTable>
	</div>
</details>

:::


<Class name="string" namespaces="std" folder="string">

## Template parameters

<Class.SymbolTable noTraits>
	<Class.TypeName name="CharT"		desc="character type"/>
	<Class.TypeName name="Traits" 		desc="traits class specifying the operations on the character type"/>
	<Class.TypeName name="Allocator"	desc={<><a href="/docs/named_req/allocator">Allocator</a> type used to allocate internal storage</>}/>
</Class.SymbolTable>

## Type names

<Class.SymbolTable>
	<Class.TypeName pub name="traits_type"				desc={<code>Traits</code>}/>
	<Class.TypeName pub name="value_type" 				desc={<code>CharT</code>}/>
	<Class.TypeName pub name="allocator_type"			desc={<code>Allocator</code>}/>
	<Class.TypeName pub name="size_type"				desc={<code>std::allocator_traits&lt;Allocator&gt;::size_type</code>}/>
	<Class.TypeName pub name="difference_type"			desc={<code>std::allocator_traits&lt;Allocator&gt;::difference_type</code>}/>
	<Class.TypeName pub name="reference"				desc={<code>value_type</code>}/>
	<Class.TypeName pub name="const_reference"			desc={<code>value_type const&amp;</code>}/>
	<Class.TypeName pub name="pointer"					desc={<code>std::allocator_traits&lt;Allocator&gt;::pointer</code>}/>
	<Class.TypeName pub name="const_pointer"			desc={<code>std::allocator_traits&lt;Allocator&gt;::const_pointer</code>}/>
	<Class.TypeName pub name="iterator"					desc={<><a href="#"><code>LegacyRandomAccessIterator</code></a> to <code>value_type</code></>}/>
	<Class.TypeName pub name="const_iterator"			desc={<><a href="#"><code>LegacyRandomAccessIterator</code></a> to <code>value_type const</code></>}/>
	<Class.TypeName pub name="reverse_iterator"			desc={<><a href="#"><code>std::reverse_iterator&lt;iterator&gt;</code></a></>}/>
	<Class.TypeName pub name="const_reverse_iterator"	desc={<><a href="#"><code>std::reverse_iterator&lt;const_iterator&gt;</code></a></>}/>
</Class.SymbolTable>

## Member functions

<Class.SymbolTable>	
	<Class.Method pub autoLink linkName="constructor" name="(constructor)">
		Constructs a <code>basic_string</code>
	</Class.Method>
	<Class.Method pub autoLink linkName="destructor" name="(destructor)">
		Destroys the string, deallocating internal storage if used
	</Class.Method>
	<Class.Method pub autoLink linkName="operator_assign" name="operator="
			desc="Assigns values to the string"
		/>
	<Class.Method pub autoLink name="assign"
			desc="Assigns characters to the string"
		/>
	<Class.Method pub autoLink name="get_allocator"
			desc="Returns the associated allocator"
		/>
</Class.SymbolTable>

### Element access

<Class.SymbolTable>	
	<Class.Method pub autoLink name="at"
			desc={<>Accesses the specified character <b>with bounds checking</b></>}
		/>
	<Class.Method pub autoLink linkName="operator_index" name="operator[]"
			desc={<>Accesses the specified character</>}
		/>
	<Class.Method pub autoLink name="front"
			desc="Returns the first character"
		/>
	<Class.Method pub autoLink name="back"
			desc="Returns the last character"
		/>
	<Class.Method pub autoLink name="data"
			desc="Returns a pointer to the first character of a string"
		/>
	<Class.Method pub autoLink name="c_str"
			desc="Returns a non-modifiable standard C character array version of the string"
		/>
	<Class.Method pub autoLink linkName="operator_basic_string_view" name="operator basic_string_view"
			desc={<>returns a non-modifiable <code>string_view</code> into the entire string</>}
		/>
</Class.SymbolTable>

### Iterators

<Class.SymbolTable>	
	<Class.Method pub autoLink linkName="begin" name={<>begin<br/>cbegin</>}
			desc={<>Returns an <code>iterator/const_iterator</code> to the beginning</>}
		/>
	<Class.Method pub autoLink linkName="end" name={<>end<br/>cend</>}
			desc={<>Returns an <code>iterator/const_iterator</code> to the end</>}
		/>
	<Class.Method pub autoLink linkName="rbegin" name={<>rbegin<br/>crbegin</>}
			desc={<>Returns an reverse <code>iterator/const_iterator</code> to the beginning</>}
		/>
	<Class.Method pub autoLink linkName="rend" name={<>rend<br/>crend</>}
			desc={<>Returns an reverse <code>iterator/const_iterator</code> to the end</>}
		/>
</Class.SymbolTable>

### Capacity

<Class.SymbolTable>	
	<Class.Method pub autoLink name="empty">
		Returns <code>true</code> if the string is empty, otherwise <code>false</code>.
	</Class.Method>
	<Class.Method pub autoLink linkName="size" name={<>size<br/>length</>}
			desc="Returns the number of characters"
		/>
	<Class.Method pub autoLink name="max_size"
			desc="Returns the maximum number of characters"
		/>
	<Class.Method pub autoLink name="reserve"
			desc="Reserves storage"
		/>
	<Class.Method pub autoLink name="capacity"
			desc="Returns the number of characters that can be held in currently allocated storage"
		/>
	<Class.Method pub autoLink name="shrink_to_fit"
			desc="Reduces memory usage by freeing unused memory"
		/>
</Class.SymbolTable>

### Operations

<Class.SymbolTable>
	<Class.Method pub autoLink name="clear"
			desc="Clears the contents"
		/>
	<Class.Method pub autoLink name="insert"
			desc="Inserts characters"
		/>
	<Class.Method pub autoLink name="erase"
			desc="Removes characters"
		/>
	<Class.Method pub autoLink name="push_back"
			desc="Appends a character to the end"
		/>
	<Class.Method pub autoLink name="pop_back"
			desc="Removes the last character"
		/>
	<Class.Method pub autoLink name="append"
			desc="Appends characters to the end"
		/>
	<Class.Method pub autoLink linkName="operator_addeq" name="operator+="
			desc="Appends characters to the end"
		/>
	<Class.Method pub autoLink name="compare"
			desc="Compares two strings"
		/>
	<Class.Method pub autoLink name="starts_with"
			desc="Checks if the string starts with the given prefix"
		/>
	<Class.Method pub autoLink name="ends_with"
			desc="Checks if the string ends with the given suffix"
		/>
	<Class.Method pub autoLink name="contains"
			desc="Checks if the string contains the given substring or character"
		/>
	<Class.Method pub autoLink name="replace"
			desc="Replaces specified portion of a string"
		/>
	<Class.Method pub autoLink name="substr"
			desc="Returns a substring"
		/>
	<Class.Method pub autoLink name="copy"
			desc="Copies characters"
		/>
	<Class.Method pub autoLink name="resize"
			desc="Changes the number of characters stored"
		/>
	<Class.Method pub autoLink name="swap"
			desc="Swaps the contents"
		/>
</Class.SymbolTable>

### Search

<Class.SymbolTable>
	<Class.Method pub autoLink name="find"
			desc="Find characters in the string"
		/>
	<Class.Method pub autoLink name="rfind"
			desc="Find the last occurrence of a substring"
		/>
	<Class.Method pub autoLink name="find_first_of"
			desc="Find first occurrence of characters"
		/>
	<Class.Method pub autoLink name="find_first_not_of"
			desc="Find first absence of characters"
		/>
	<Class.Method pub autoLink name="find_last_of"
			desc="Find last occurrence of characters"
		/>
	<Class.Method pub autoLink name="find_last_not_of"
			desc="Find last absence of characters"
		/>
</Class.SymbolTable>

## Constants

<Class.SymbolTable>
	<Class.Symbol pub static constexpr name="npos"
			desc="Special value. The exact meaning depends on the context"
		/>
</Class.SymbolTable>

## Non-member functions

<Class.SymbolTable noTraits>
	<Class.Method name="operator+"
			desc="Concatenates two strings or a string and a char"
		/>
	<Class.Method name={<>
				operator==<br/>
				operator!=<br/>
				operator&lt;<br/>
				operator&gt;<br/>
				operator&lt;=<br/>
				operator&gt;=<br/>
				operator&lt;=&gt;
			</>}
			desc="Lexicographically compares two strings"
		/>
	<Class.Method name={<>std::swap <small>(std::basic_string)</small></>}
			desc={<>Specializes the <a href="#">std::swap</a> algorithm</>}
		/>
	<Class.Method name={<>
				erase <small>(std::basic_string)</small><br/>
				erase_if <small>(std::basic_string)</small>
			</>}
			desc="Erases all elements satisfying specific criteria"
		/>
</Class.SymbolTable>

### Input/output

<Class.SymbolTable noTraits>
	<Class.Method name={<>
				operator&lt;&lt;<br/>
				operator&gt;&gt;
			</>}
			desc="Performs stream input and output on strings"
		/>
	<Class.Method name="getline"
			desc="read data from an I/O stream into a string"
		/>
</Class.SymbolTable>

### Numeric conversions

<Class.SymbolTable noTraits>
	<Class.Method name={<>
				stoi<br/>
				stol<br/>
				stoll
			</>}
			desc="Converts a string to a signed integer"
		/>
	<Class.Method name={<>
				stoui<br/>
				stoul<br/>
				stoull
			</>}
			desc="Converts a string to an unsigned integer"
		/>
	<Class.Method name={<>
				stof<br/>
				stod<br/>
				stold
			</>}
			desc="Converts a string to a floating point value"
		/>
	<Class.Method name="to_string"
			desc={<>Converts an integral or floating point value to <code>std::string</code></>}
		/>
	<Class.Method name="to_wstring"
			desc={<>Converts an integral or floating point value to <code>std::wstring</code></>}
		/>
</Class.SymbolTable>

## Literals

<small>Defined in inline namespace <code>std::literals::string_literals</code></small>
<Class.SymbolTable noTraits>
	<Class.Method name='operator ""s'
			desc={<>Converts a character array literal to <code>std::string</code></>}
		/>
</Class.SymbolTable>

## Helper classes

<Class.SymbolTable noTraits>
	<Class.Method name={<>
				std::hash<small>&lt;std::string&gt;</small><br/>
				std::hash<small>&lt;std::u8string&gt;</small><br/>
				std::hash<small>&lt;std::u16string&gt;</small><br/>
				std::hash<small>&lt;std::u32string&gt;</small><br/>
				std::hash<small>&lt;std::wstring&gt;</small><br/>
				std::hash<small>&lt;std::pmr::string&gt;</small><br/>
				std::hash<small>&lt;std::pmr::u8string&gt;</small><br/>
				std::hash<small>&lt;std::pmr::u16string&gt;</small><br/>
				std::hash<small>&lt;std::pmr::u32string&gt;</small><br/>
				std::hash<small>&lt;std::pmr::wstring&gt;</small><br/>
			</>}
			desc="hash support for strings"
		/>
</Class.SymbolTable>
</Class>


## Deduction guides

<ImproveSection />

## Examples

### Basic manipulation

#### Creating and printing a simple string

<Columns spacing="20px" columns={['8fr', '4fr']}>
<div>

```cpp
#include <iostream>
#include <string>

int main()
{
	std::string s = "World";
	std::cout << "Hello, " << s << "!" << std::endl;
}
```

</div>
<div>

```plaintext title="Result"
Hello, World!
```

</div>
</Columns>

#### Concatenating strings

<Columns spacing="20px" columns={['8fr', '4fr']}>
<div>

```cpp
#include <iostream>
#include <string>

int main()
{
	std::string a = "Hello, ";
	std::string b = "World!";
	std::string c = a + b;
	std::cout << c;
}
```

</div>
<div>

```plaintext title="Result"
Hello, World!
```

</div>
</Columns>

#### Splitting a string

<Columns spacing="20px" columns={['8fr', '4fr']}>
<div>

```cpp
#include <iostream>
#include <string>

int main()
{
	// Split by the comma.
	//                    v
	std::string a = "Hello, World!";
	size_t pos = a.find(',');

	std::string hello = a.substr(0, pos);
	// 1 for a comma, and 1 for a space before "World"
	std::string world = a.substr(pos + 1 + 1);

	std::cout << hello << '\n' << world;
}
```

</div>
<div>

```plaintext title="Result"
Hello
World!
```

</div>
</Columns>


### Conversions

#### Convert `std::string` to `int`

<Columns spacing="20px" columns={['8fr', '4fr']}>
<div>

```cpp
#include <iostream>
#include <string>

int main()
{
	std::string numberInString = "8314";

	// Note: stoi can throw exception if failed!
	int number = std::stoi(numberInString);
	std::cout << "Number: " << number;
}
```

</div>
<div>

```plaintext title="Result"
Number: 8314
```

</div>
</Columns>

#### Convert `std::string` to `float`

<Columns spacing="20px" columns={['8fr', '4fr']}>
<div>

```cpp
#include <iostream>
#include <string>

int main()
{
	std::string numberInString = "3.141";

	// Note: stof can throw exception if failed!
	float number = std::stof(numberInString);
	std::cout << "Number: " << number;
}
```

</div>
<div>

```plaintext title="Result"
Number: 3.141
```

</div>
</Columns>
